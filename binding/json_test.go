...
func TestName(t *testing.T) {
	...
	person1 := Person{}
	err := ParseJsonToInstance([]byte(test1), &person1)
	if err != nil {
		panic(err.Error())
	}
	test2 := `{"age": 27, "info": "hello123", "mobile": "15600634756"}`
	person2 := Person{}
	er := ParseJsonToInstance([]byte(test2), &person2)
	if er != nil {
...
func TestMobile(t *testing.T) {
	...
	person3 := Person{}
	e := ParseJsonToInstance([]byte(test3), &person3)
	if e != nil {
		fmt.Println(e.Error())
	} else {
		panic("test case failed")
...
func TestInfo(t *testing.T) {
	...
	person2 := Person{}
	er := ParseJsonToInstance([]byte(test2), &person1)
	if er != nil {
		panic(er.Error())
	}
